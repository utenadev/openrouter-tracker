...
### 11. ハイブリッドデータ取得
- **概要**: テーブル形式とリスト形式のデータを組み合わせて完全なデータセットを取得する
- **目的**: 39モデルの完全なデータ（週間トークン数を含む）を取得する
- **課題**: 
  - リスト形式パーサーの実装
  - データ統合ロジックの実装
  - エラーハンドリングの強化
...
- **対応**: 
  ...
  - メイン関数の更新
- **優先度**: 高（設計済み、実装待ち）
- **状態**: 設計済み（HYBRID_DATA_FETCHING_DESIGN.mdを参照）
- **設計ドキュメント**: docs/HYBRID_DATA_FETCHING_DESIGN.md
- **メリット**: 
  - 完全なデータセット（39モデル + 週間トークン数）
  - 柔軟性（片方の形式が失敗しても動作）
  - 拡張性（新しい形式にも対応可能）
  - 信頼性（データの信頼性向上）
- **デメリット**: 
